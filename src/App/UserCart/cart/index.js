...
export const useCart = (idCart, cartObject) => {
  ...
  const queryCart = db.collection("catalog-user-data").doc(cart.buyerStoreownerId).collection("cart").doc(cart.id)
  ...
  const cartRef = useMemo(
    () =>
      cart.buyerStoreownerId &&
      db.collection("catalog-user-data").doc(cart.buyerStoreownerId).collection("cart").doc(cart.id),
    [],
  )
...
const findCart = (cart, id) =>
  (Object.entries(cart || {}).find(([_cartId, { paymentId }]) => paymentId === id) || [null])[0]
