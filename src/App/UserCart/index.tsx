...
export default ({ cartId }) => {
  const { fantasia } = useContext(userContext)
  ...
  useEffect(() => {
    if (allCarts.length > 0 && allStoreowners.length > 0) {
      ...
      console.log(
        "allCarts.reduce(toCartArray, [])",
        allCarts
          .reduce(toCartArray, [])
          .filter(item => "QBONITA VESTIDOS DE FESTA EIRELI".includes(item.brandName.toUpperCase())),
      )
      console.log(
        "allCarts.reduce(toCartArray, [])",
        allCarts.reduce(toCartArray, []).filter(item => fantasia.includes(item.brandName.toUpperCase())),
      )
      setCarts(allCarts.reduce(toCartArray, []).filter(item => fantasia.includes(item.brandName.toUpperCase())))
  ...
  if (cartId && selectedCart && selectedStoreowner)
    return <CartItem state={state} cart={selectedCart} storeowner={selectedStoreowner} oldQuery={queryStr} />
