...
const GerarBoleto = ({ boletbankId, boletId }) => {
  const { zoopId, docId, razao, email, fantasia } = useContext(userContext)
  ...
  const state = {
    ...
    setFinishPending,
    paymentDuplicatas,
    setPaymentDuplicatas,
    paymentBoletos,
    setPaymentBoletos,
    pendingBoletos,
    setPendingBoletos,
    pendingDuplicatas,
    setPendingDuplicatas,
    seller: fantasia,
    sellerId: zoopId,
    razao,
    charge,
  ...
  if (boletbankId && boletId)
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <DuplicataDetails
          transactions={firstTicket}
          boletbankId={boletbankId}
          boletId={boletId}
  ...
  if (boletbankId)
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <DuplicataDetails transactions={firstTicket} boletbankId={boletbankId} boletId="" sellerId={state.sellerId} />
      </motion.div>
    )
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <RelatorioList transactions={ticket} />
    </motion.div>
  )
}
