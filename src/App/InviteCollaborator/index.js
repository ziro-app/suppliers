...
const InviteCollaborator = () => {
  ...
  const { uid, fantasia } = useContext(userContext)
  const state = {
    uid,
    supplier: capitalize(fantasia),
    fname,
    lname,
    email,
    role,
  ...
  const validations = [
    ...
    {
      name: "brands",
      validation: value => (fantasia === "ZIRO" ? catalogBrands.includes(value) : true),
      value: brand,
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      style={{ display: "grid", gridTemplateRows: "auto 1rm", gridRowGap: "20px" }}
    >
      <>
        {fantasia === "ZIRO" ? (
          <Form
