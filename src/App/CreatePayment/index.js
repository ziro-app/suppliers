...
const CreatePayment = () => {
  const { fantasia, zoopId, docId, role, fname, brand, maxInstallments } = useContext(userContext)
  ...
  useEffect(() => {
    async function getSellerZoopPlan() {
      await db
        ...
        .where("fantasia", "==", fantasia.toUpperCase())
        .onSnapshot(snap => {
          if (!snap.empty) {
            snap.forEach(doc => {
  ...
  useEffect(() => {
    if (alwaysInsuredToggle === true && afterBackend === true) {
      setInsurance(true), setInsurenceDropdownValue("Com seguro")
  ...
  return hasSellerZoopPlan &&
    (Object.prototype.hasOwnProperty.call(hasSellerZoopPlan, "activePlan")
      ? true
      : Object.prototype.hasOwnProperty.call(hasSellerZoopPlan.antiFraud, "amount") ||
      Object.prototype.hasOwnProperty.call(hasSellerZoopPlan.antiFraud, "percentage")) ? (
        ...
        <Form
          ...
          inputs={[
            <FormInput
              ...
              input={<InputMoney value={charge} setValue={setCharge} />}
            />,
            <FormInput
              name="installmentsMax"
              ...
              input={
                <Dropdown
                  ...
                  value={
                    ...
                    (alwaysInsuredToggle === true && afterBackend === false ? "Com seguro" : "")
                  }
                  onChange={({ target: { value } }) => {
                    if (value === "Com seguro") {
                      setInsurance(true)
                  ...
                  onChangeKeyboard={element => {
                    if (element === null) {
                    } else if (element.value === "Com seguro") {
                      setInsurance(true)
                      setInsurenceDropdownValue("Com seguro")
          ...
          <div style={{ display: "grid", gridRowGap: "2px" }}>
            <label style={{ fontSize: window.innerWidth < 400 ? "1.3rem" : "1.4rem", opacity: "0.85" }}>
              {window.innerWidth < 400
                ? "Deseja mais confiança nas vendas?"
                : "Deseja mais confiabilidade em suas vendas?"}
            </label>
            <label style={{ fontSize: window.innerWidth < 400 ? "1.3rem" : "1.4rem", opacity: "0.85" }}>
              ...
              <label
                ...
                style={{
                  cursor: "pointer",
                  color: primaryColor,
                  fontSize: window.innerWidth < 400 ? "1.3rem" : "1.4rem",
      ...
      <div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <label style={{ fontSize: window.innerWidth < 400 ? "1.3rem" : "1.4rem" }}>
              Deseja mais confiabilidade em suas vendas?
            </label>
            <label style={{ fontSize: window.innerWidth < 400 ? "1.3rem" : "1.4rem" }}>
              Consulte CPF/CNPJ do seu cliente{" "}
              <label
                onClick={() => setLocation("/consulta")}
                style={{
                  cursor: "pointer",
