    ...
    try {
      ...
      if (seller && sellerId) {
        ...
        if (isCollaborator) {
          ...
          if (isValid) {
            ...
            if (hasSellerZoopPlan) {
              const fetchedStandardPlan = await db
                .collection("suppliers")
                .where("fantasia", "==", seller.toUpperCase())
                .get()
        ...
        try {
          ...
          if (doc)
            await navigator.clipboard.writeText(linkMessage(baseUrl, doc.id, seller, formattedCharge, installmentsMax))
        } catch (error) {
