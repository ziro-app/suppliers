...
const completeRegistration = state => () => {
  ...
  const fantasiaSheet = fantasias.filter(
    item => item.cnpj === Number(cnpj.replace(".", "").replace(".", "").replace("/", "").replace("-", "")),
  )
            ...
            try {
              try {
                ...
                const {
                  ...
                  `https://ziro-pay.netlify.app/.netlify/functions/sellers-create`,
                  {
                    ein: cnpj,
                    owner: {
                ...
                try {
                  ...
                  try {
                    ...
                    await Promise.all(
                      [fileDoc, fileAtv, fileRes, fileCnpj].map(async (file, index) => {
                        try {
                          ...
                          throw {
                            msg: `Erro no upload da imagem ${index + 1}, fale com seu assessor.`,
                            customError: true,
                          }
                        }
                      }),
                    ...
                    try {
                      ...
                      const sellerFirebaseData: IFirebaseData = {
                        origin: "firebase",
                        collection: "suppliers",
                        field: "uid",
                        identifier: user.uid,
                      ...
                      const usersFirebaseData: IFirebaseData = {
                        origin: "firebase",
                        collection: "users",
                        field: "email",
                        identifier: email,
