...
const sendToBackend = state => () => {
  const { zoopBankAccountId, currentBalance, redeemBalance, setCurrentBalance, setRedeemBalance } = state
      ...
      if (redeemParsed <= currentBalance) {
        if (zoopBankAccountId) {
          try {
            ...
            await post(
              `https://ziro-pay.netlify.app/.netlify/functions/transfer-create?bank_account_id=${zoopBankAccountId}`,
              {
