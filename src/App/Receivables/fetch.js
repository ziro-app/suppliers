...
const fetch = (
  ...
  const run = async () => {
    try {
      ...
      let arrayItems = {}
      while (hasMore) {
        const url = `https://ziro-pay.netlify.app/.netlify/functions/sellers-future-releases?seller_id=${zoopId}&expected_on_range[gte]=${parsedToday}&expected_on_range[lte]=${parsedFnDay}&offset=${offset}`
          ...
          if (vendas > 0) {
            ...
            const val =
              normalizedArray.length > 0
                ? parseFloat(normalizedArray.map(it => it.net).reduce((a, b) => reducerTotal(a, b)) / 100).toFixed(2)
                : "0"
        ...
        setData([
          {
            ...
            rowsClicks:
              dataTable[0] && dataTable[0].rowsClicks ? [...dataTable[0].rowsClicks, ...rowsClicks] : rowsClicks,
