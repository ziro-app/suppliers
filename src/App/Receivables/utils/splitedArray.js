...
const splitedArray = async (array, fantasy, zoopId) => {
    const result = array.filter(({ installment_plan, payment_type }) => installment_plan || payment_type === 'commission').map(({ id, installment_plan, fees, payment_type, amount, net }) => ({ id, installment_plan, fees, payment_type, amount, net }));
    const firebaseObjs = await Promise.all(result.map(async ({ id, installment_plan, fees, payment_type, amount, net }) => {
        if (payment_type === 'commission') {
            return {
                id,
                amount: `${parseFloat(parseInt(amount) / 100).toFixed(2)}`.replace('.', ''),
                fees: `${parseFloat(parseInt(fees) / 100).toFixed(2)}`.replace('.', ''),
                net: `${parseFloat(parseInt(net) / 100).toFixed(2)}`.replace('.', ''),
                commission: true
            };
        } else {
    ...
    firebaseObjs
        .filter(it => it !== null)
        .map(it => {
            const { id, amount, antifraud, installment_plan, commission,
                markup, net, ziroPay, fees } = it;
            if (commission) {
                if (!test[id]) {
                    test[id] = it;
                } else {
                    test[id]['amount'] = `${parseInt(test[id]['amount']) + parseInt(amount)}`;
                    test[id]['fees'] = `${parseInt(test[id]['fees']) + parseInt(fees)}`;
