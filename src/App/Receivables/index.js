...
const Receivables = ({ receivableId }) => {
  ...
  const { zoopId, payoutAutomatic, fantasia } = useContext(userContext)
  ...
  if (receivableId && receivableId !== "dados-bancarios")
    return <Transactions receivableId={receivableId} receivables={receivables} snapshot={state} />
    ...
    <Menu title="Recebíveis">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <div style={{ display: "grid", gridRowGap: "20px" }}>
          <Button
            cta="Configurar dados bancários"
            style={btn}
            navigate={() => setLocation("/dados-bancarios")}
            type="link"
          />

          {payoutAutomatic != undefined && payoutAutomatic != null && payoutAutomatic === false && (
            <Button cta="Resgatar saldo" style={btn} navigate={() => setLocation("recebiveis/resgate")} type="link" />
          )}

          {receivables.length > 0 && totalAmount > 0 && totalTransactions > 0 && (
            <Button
              cta="Exportar recebíveis futuros"
              style={btn}
              click={() => convertCsv(receivables, totalAmount, totalTransactions, "Recebiveis Futuros.csv")}
              type="button"
            />
          )}
          ...
          <Table data={data} cellStyle={cellStyle} customGrid={customGrid} />
