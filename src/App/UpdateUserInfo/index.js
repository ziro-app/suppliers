...
const UpdateUserInfo = () => {
  ...
  const [newFantasy, setNewFantasy] = useState(fantasia !== "undefined" ? fantasia : "")
  ...
  const [newWhatsApp, setNewWhatsApp] = useState(
    whatsApp && whatsApp.startsWith("55 ") ? whatsApp.replace("55 ", "") : whatsApp,
  )
  ...
  const validatePhone = () => {
    if (
      newPhone.length <= 14 && newPhone !== ""
        ? /(^\(\d{2}\) \d{4}\-\d{4}$)/.test(newPhone)
        : newPhone.length === 15
          ? /(^\(\d{2}\) \d{5}\-\d{4}$)/.test(newPhone)
          : true
    ) {
      setErrorPhone("")
      return true
  ...
  return (
    ...
    >
      <label style={{ fontSize: "1.5rem", marginBottom: "10px", textAlign: "center" }}>
        Você pode atualizar dados do seu CNPJ pelo botão abaixo, caso já estejam na Receita Federal
      </label>
      ...
      <InputEdit name="CNPJ" value={newCnpj} setError={() => { }} editable={false} />
      <InputEdit
        name="Razão Social"
        value={newReason}
      ...
      <InputEdit
        ...
        onChange={({ target: { value } }) =>
          setNewPhone(maskInput(value, value.length <= 14 ? "(##) ####-####" : "(##) #####-####", true))
        }
      ...
      <div style={{ paddingTop: "25px" }}>
        ...
        <div style={{ padding: "5px 0px", textAlign: "left" }}>
          <label style={help}>
            ...
            <label onClick={() => setLocation("/minha-conta")} style={helpLink}>
              acesse aqui
            </label>
            .
          </label>
        ...
        <div style={{ padding: "5px 0px", textAlign: "left" }}>
          <label style={help}>
            ...
            <label onClick={() => setLocation("/recebiveis/dados-bancarios")} style={helpLink}>
              acesse aqui
            </label>
            .
          </label>
