...
const _InputEmail = (globalState?: { useState: () => any }) => {
  const InputEmail = ({ inputName, value, setValue, isLoading = false, inputError, isDisabled = false, styleErrorIcon, styleErrorText, style, ...props }: InputCommonProps) => {
    if (globalState) {
    ...
    return (
      <div style={container}>
        ...
        <input
          ...
          onChange={e => (globalState ? setGlobalValue({ userInput: e.target.value.toLowerCase().replace(/\s/g, '') }) : setValue && setValue(e.target.value.toLowerCase().replace(/\s/g, '')))}
