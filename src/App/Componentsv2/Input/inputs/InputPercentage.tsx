...
const _InputPercentage = (globalState?: { useState: () => any }) => {
  const InputPercentage = ({ inputName, value, setValue, isLoading = false, inputError, isDisabled = false, styleErrorIcon, styleErrorText, style, ...props }: InputCommonProps) => {
    if (globalState) {
    ...
    return (
      <div style={container}>
        ...
        <input
          ...
          onChange={e => (globalState ? setGlobalValue({ userInput: percentageToInteger(e.target.value) }) : setValue && setValue(percentageToInteger(e.target.value)))}
