...
const formatReceivablesSheet = (transactionZoopId: string, receivables: Receivable[]) =>
...
const simplifyFeeDetails = (fee_details: RegisteredTransaction.Response["fee_details"]) => {

    return [...fee_details].map(({ amount, type, description }) => ({ amount, type, description }));
};
...
export default (
    ...
    const feeDetailsDB = simplifyFeeDetails(fee_details);
    ...
    const receivablesSheet = formatReceivablesSheet(transactionId, receivablesDB);
    return {
        receivablesDB,
        feeDetailsDB,
        total,
