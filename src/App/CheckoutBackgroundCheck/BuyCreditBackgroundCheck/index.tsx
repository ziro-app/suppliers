...
import { createPayment, getReceivables } from "./utils/functionsZoop"
...
const redeMessageFinder = (error: any) => ({ additionalData }: RedeMessage) =>
  error.response_code === additionalData.response_code
...
const BuyCreditBackgroundCheck = ({ setPaidRequests }) => {
  ...
  const header = useMemo(
    () => <HeaderWithoutModal title="Finalizar" leftButton={{ icon: "close", onClick: close }} />,
    [close],
  )
  ...
  const buyer = { zoopId, docId, fantasia, reason, quantityNumber }
  console.log(buyer)
  const {
    cardholder,
    expiry,
  ...
  const state = {
    ...
    prettyNumber,
    cvv,
    totalValueCreditBackgroundCheck,
    docId,
    isCollaborator,
    ownerId,
    document,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div style={{ display: "grid", textAlign: "center", justifyContent: "center", paddingBottom: "20px" }}>
        <strong style={{ fontSize: "1.5rem", fontFamily: "Rubik", color: "#000" }}>
          ...
          {parseFloat(`${totalValueCreditBackgroundCheck}`)
          ...
          /*{() =>
            ...
            const promise: any = new Promise(async resolve => {
              try {
                ...
                const [dbData, sheetData, receivablesData] = dbAndSheet(transaction, buyer, receivables, timestamp)
