...
const mountSplitRules = ({ sellerZoopPlan, cardBrand, installments, insurance, split_rules }) => {
    try {
        ...
        let filterSplitAntiFraud =
            split_rules.length > 0 && insurance === true ? split_rules.filter(item => parseFloat(item.percentage) === parseFloat(percentageAntiFraud)) : [{ percentage: 0, amount: 0 }];
        if (typeof filterSplitAntiFraud[0] === 'undefined') filterSplitAntiFraud = [{ percentage: 0, amount: 0 }];
        let filterSplitMarkup = split_rules.length > 0 ? split_rules.filter(item => parseFloat(item.percentage) === parseFloat(percentageMarkup)) : [{ percentage: 0, amount: 0 }];
