...
const fetch = (
  ...
  ownerId,
  setCustomError,
  { setFreeRequests, setPaidRequests },
) => {
  const run = async () => {
    try {
      ...
      if (doc.exists) {
        const {
          ...
          backgroundCheckCurrentMonth,
          backgroundCheckCurrentYear,
        } = doc.data()
        const now = new Date()
        ...
        // Requisição no mês diferente do corrente
          ...
          await db
            ...
            .update({
              ...
              backgroundCheckCurrentMonth: now.getMonth(),
              backgroundCheckCurrentYear: now.getFullYear(),
            })
          reqFree = 10
        }
      }
