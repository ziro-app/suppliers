...
export const App = () => {
    ...
    const fillObject = (data) => {
        const { cnpj, endereco, bairro, cep, cidade, estado, codBanco, titular, tipoConta,
            numConta, agencia, fantasia, razao, zoopId } = data;
        setCnpj(cnpj ? cnpj : '');
                        ...
                        if (!snapshot.empty) {
                            ...
                            unsubscribe = db
                                ...
                                .onSnapshot(async snapshot => {
                                    if (!snapshot.empty) {
                                        const { email, fname, lname, ownerId, role } = snapshot.docs[0].data();
                    ...
                    if (!docRef.empty) {
                        docRef.forEach(async doc => {
                            ...
                            if (userPos === null || userPos === '')
                                setUserPos(await findStoreownerRow(cnpj));
                        });
                    } else {
                        const docCollaboratorRef = await db.collection('collaborators').where('uid', '==', uid).get();
                        const { email, fname, lname, ownerId, role } = docCollaboratorRef.docs[0].data();
                        const supplierInfo = await db.collection('suppliers').doc(ownerId).get();
                        ...
                        fillObject(supplierInfo.data());
                        if (userPos === null || userPos === '')
                            setUserPos(await findCollaboratorRow(email));
                    }
                } catch (error) {
