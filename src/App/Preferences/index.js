...
const Preferences = () => {
  const { docId, role, maxInstallments, paymentsInsurance, fantasia } = useContext(userContext)
    ...
    role === "" && (
      <div>
        ...
        >
          <div>
            ...
            <div style={{ display: "inline-block", marginLeft: "8px" }}>
              <TooltipHelp body={tooltipSeguro} title={tituloSeguro} illustration="security" />
            </div>
          </div>
          <div>
            <ToggleButton active={insuranceValue} onClick={handleToggle} size={30} template="primary" />
          </div>
        </div>

        <div>
          <div style={{ marginTop: "30px" }}>
            <label style={{ fontSize: window.innerWidth < 400 ? "1.4rem" : "1.6rem" }}>
              Parcelamento máximo por venda
            </label>
            <div style={{ display: "inline-block", marginLeft: "8px" }}>
              <TooltipHelp body={tooltipParcelamento} title={tituloParcelamento} illustration="creditCard" />
            </div>
          </div>
        ...
        <div style={{ paddingTop: "30px" }}>
          <Button type="button" cta="Salvar preferências" click={sendToBackend(state)} />
        </div>
