...
const Transactions = ({ transactionId, receivableId, setTransactionId }) => {
    ...
    if (transactionId && receivableId) return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}><ReceivableDetails transactions={payments} transactionId={transactionId} receivableId={receivableId} /></motion.div>;
    if (transactionId) setTransactionId(transactionId);
    if (transactionId) return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}><TransactionDetails transactions={payments} transactionId={transactionId} /></motion.div>;
    return (
