...
function Upgrade() {
  const { zoopId, docId, cnpj, reason, userPos, typeRegister } = useContext(userContext);
  ...
  const state = { category, categoryName, cpf, dayOfBirth, phone, accountType, accountTypeViewName, bankName, bankNumber, agency, accountNumber, zoopId, docId, cnpj, reason, userPos, idDoc, idAtv, idRes, idCnpj };
  ...
  return (
    <div style={container}>
      ...
      {typeRegister === "Simplificado" && step === 1 &&
        <>
        <p style={{ textAlign: 'center', fontSize: '1.6rem', color: 'rgb(34, 34, 34)', marginTop: '10px' }}>Preencha os dados abaixo e qualquer dúvida fale conosco. <TooltipHelp illustration='chatting' title={supportModalTitle} body={supportModalBody} iconColor='#2D9CDB' iconSize={20} supportButton /></p>
