...
const sendToBackend = (state) => () => {
  const { category, categoryName, cpf, dayOfBirth, phone, accountType, accountTypeViewName, bankName, bankNumber, agency, accountNumber, zoopId, docId, cnpj, reason, userPos, idDoc, idAtv, idRes, idCnpj } = state;
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        await updateSellerZoop(zoopId, cpf, dayOfBirth, phone, category);
      }catch(error){
        if (error.customError) throw error
